...
bool ScrollArea::viewportEvent(QEvent *e) {
	if (filterOutTouchEvent(e)) {
		...
		if (ev->phase() == Qt::NoScrollPhase) {
			const auto multiply = ev->modifiers()
				& (Qt::ControlModifier | Qt::ShiftModifier);
			if (IsSmoothScrollingEnabled() && !multiply) {
				const auto delta = ScrollDelta(ev);
				const auto vertical = _wheelDirectionLocked
					|| (std::abs(delta.y()) >= std::abs(delta.x()));
				const auto scrollDelta = vertical ? -delta.y() : -delta.x();
				auto bar = vertical ? verticalScrollBar() : horizontalScrollBar();
				if (bar && bar->maximum() > bar->minimum() && scrollDelta != 0) {
					const auto weak = base::make_weak(this);
					const auto current = (_smoothScrollAnimation.animating()
						&& !base::OppositeSigns(_smoothScrollTarget - bar->value(), scrollDelta))
						? _smoothScrollTarget
						: bar->value();
					_smoothScrollTarget = std::clamp(
						current + scrollDelta,
						bar->minimum(),
						bar->maximum());
					if (_smoothScrollTarget != bar->value()) {
						...
						_smoothScrollAnimation.start(
							[=](float64 val) {
								...
								const auto rounded = int(base::SafeRound(val));
