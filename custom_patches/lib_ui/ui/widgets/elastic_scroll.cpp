	...
	if (phase == Qt::NoScrollPhase) {
		if (_overscroll == currentOverscrollDefault()) {
			...
			if (!smooth) {
				...
				const auto from = (_smoothScrollAnimation.animating()
					&& !base::OppositeSigns(_smoothScrollTarget - _state.visibleFrom, delta))
					? _smoothScrollTarget
					: _state.visibleFrom;
				const auto desired = from + delta;
				requestBottomContent(desired - _state.visibleFrom);
				if (!weak) {
					return true;
				}
