...
bool UrlRequiresConfirmation(const QUrl &url) {
	if (CustomFeatures::GetConfig().directExternalLinks) {
		return false;
	}
	using namespace qthelp;
...
QString HiddenUrlClickHandler::copyToClipboardText() const {
	auto original = originalUrl();
	if (CustomFeatures::GetConfig().cleanTrackingUrls) {
		original = CustomFeatures::CleanTrackingParameters(original);
	}
	...
	auto value = url();
	if (CustomFeatures::GetConfig().cleanTrackingUrls) {
		value = CustomFeatures::CleanTrackingParameters(value);
	}
...
void HiddenUrlClickHandler::Open(QString url, QVariant context) {
	if (CustomFeatures::GetConfig().cleanTrackingUrls) {
		url = CustomFeatures::CleanTrackingParameters(url);
	}
